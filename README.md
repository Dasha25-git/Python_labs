...
В этой работе я написала автотесты для функций из предыдущих лабораторных и настроила форматирование кода с помощью black.

## Задание 1 - test_text

В начале подключаю sys и Path. Через них вычисляю путь к корню проекта (PROJECT_ROOT) и к папке src (SRC_DIR). Если src ещё нет в sys.path, добавляю её вручную. Это нужно, чтобы Python видел модули из папки src, даже если тесты запускаются отдельно. Далее импортирую pytest и из модуля lab03.text функции: normalize, tokenize, count_freq, top_n.

Ниже идут тесты для normalize: используется @pytest.mark.parametrize, чтобы одной тестовой функцией проверить сразу несколько случаев. Я проверяю:
- перевод строки и табы заменяются пробелами
- буквы приводятся к нижнему регистру
- Ё заменяется на Е
- лишние пробелы в начале, в конце и между словами убираются

В функции test_normalize_basic просто сравниваю результат normalize(source) с ожидаемой строкой expected.
![Test 1](images/lab07/test1.png)

Функция test_tokenize_basic проверяет, что tokenize(source) возвращает именно такой список, который указан в expected_tokens.
![Test 1](images/lab07/test2.png)

В test_top_n_basic создаю словарь частот {"a": 3, "b": 1, "c": 2}, вызываю top_n(freq, 2) и проверяю, что результатом становится список [("a", 3), ("c", 2)], то есть два самых частых слова в правильном порядке. В test_top_n_tie_breaker проверяю особый случай из задания — когда частоты одинаковые:
- беру словарь {"banana": 2, "apple": 2, "cherry": 1},
- вызываю top_n(freq, 2)
- ожидаю, что при одинаковой частоте слова сортируются по алфавиту: сначала ("apple", 2), потом ("banana", 2)

![Test 1](images/lab07/test3.png)

Запускаю тесты:

![Test 1](images/lab07/output1.png)

## Задание 2 - test_json_csv

### Положительные тесты

Функция test_json_to_csv_roundtrip: с помощью фикстуры tmp_path создаю два временных файла: people.json и people.csv; формирую список словарей с данными о людях и записываю его в JSON; вызываю json_to_csv, чтобы сконвертировать JSON в CSV; открываю получившийся CSV с помощью csv.DictReader; проверяю, что:
- количество строк в CSV совпадает с длиной исходного списка
- среди заголовков есть ключи name и age
Функция test_csv_to_json_roundtrip делает обратную проверку: создаётся CSV-файл с колонками name и age; вызывается csv_to_json, который пишет JSON; полученный JSON читается через json.loads; сравнивается количество записей и наличие ключей name и age в итоговых словарях.

![Test 2](images/lab07/test4.png)

### Негативные тесты

Функция test_json_to_csv_empty_file: создаётся пустой файл empty.json; при вызове json_to_csv ожидаю, что будет выброшено исключение ValueError. 
Функция test_csv_to_json_empty_file: аналогичный тест для пустого CSV-файла и функции csv_to_json.
Функция test_json_to_csv_file_not_found: путь к JSON-файлу задаётся, но сам файл не создаётся; ожидается исключение FileNotFoundError, так как входного файла не существует.
Функция test_csv_to_json_file_not_found: аналогично для отсутствующего CSV-файла и функции csv_to_json.

![Test 2](images/lab07/test5.png)

Запускаю тесты:

![Test 2](images/lab07/output2.png)

## Задание 3

black прошёл по всем Python-файлам в проекте и автоматически отформатировал их по единому стилю. В выводе видно, сколько файлов было изменено. В результате написано, что все файлы «would be left unchanged», то есть проект полностью соответствует требованиям форматтера.

![Test 3](images/lab07/output3.png)

# Лабораторная работа 8

В этой работе я описала класс Student, представляющий студента.

## Файл models.py

Класс Student реализован с использованием декоратора @dataclass, что автоматически создаёт конструктор. Класс содержит: ФИО студента, дата рождения, учебная группа и средний балл.

В методе __post_init__ выполняется проверка корректности входных данных: проверяется формат даты рождения (YYYY-MM-DD) и что средний балл находится в диапазоне от 0 до 5.

Метод age() вычисляет возраст студента в полных годах на основе текущей даты и даты рождения.

Метод to_dict() преобразует объект Student в словарь (для JSON потом).

Метод  from_dict() создаёт объект Student из словаря, полученного при чтении данных из файла.

![Task1.1](images/lab08/task1.png)
![Task1.2](images/lab08/task2.png)

## Файл serialize.py

Функция students_to_json() принимает список объектов Student и путь к файлу. Каждый объект преобразуется в словарь с помощью метода to_dict(), после чего данные сохраняются в JSON-файл.

Функция students_from_json() выполняет обратную операцию. Она считывает данные из JSON-файла, преобразует каждый словарь в объект Student с помощью метода from_dict() и возвращает список объектов студентов.

![Task2](images/lab08/task3.png)

## Файл students.json

Данный файл содержит сериализованные данные студентов в формате JSON.

## Файл test.py

Тестируем написанные функции на тестовых данных.

![TaskTest1](images/lab08/task4.png)
![TaskTest2](images/lab08/task5.png)

# Лабораторная работа 9

## Файл group.py

Класс Group предназначен для работы с данными студентов, хранящимися в CSV-файле.

В конструкторе класса (__init__) принимается путь к CSV-файлу.
Если файл не существует, он автоматически создаётся, и в него записывается строка заголовков с полями: fio, birthdate, group, gpa.

Метод _read_all() считывает все строки из CSV-файла с помощью csv.DictReader и возвращает список словарей, где каждый словарь соответствует одному студенту.

Метод list() преобразует считанные строки CSV в список объектов класса Student. Для каждого словаря создаётся объект Student.

Метод add() добавляет нового студента в CSV-файл. Данные объекта Student записываются в файл в виде новой строки.

Метод find() выполняет поиск студентов по подстроке в ФИО. Поиск осуществляется без учёта регистра, и результатом является список найденных записей.

Метод remove() удаляет студента из CSV-файла по ФИО. Для этого файл полностью перечитывается, после чего в него записываются только те записи, которые не совпадают с указанным ФИО.

Метод update() обновляет данные студента по ФИО. Он позволяет изменить одно или несколько полей записи, после чего CSV-файл перезаписывается с обновлёнными данными.

![Task1.1](images/lab09/task1.png)
![Task1.2](images/lab09/task2.png)
![Task1.3](images/lab09/task3.png)

## Файл test.py

В файле test.py выполняется проверка корректности работы класса Group.

![Test1](images/lab09/test1.png)
![Test2](images/lab09/test.png)

# Лабораторная работа 10

## Файл structures.py

В данном файле реализованы две структуры данных: стек (Stack) и очередь (Queue).

### Класс Stack

Класс Stack реализует структуру данных «стек» на основе обычного списка Python.

В конструкторе (__init__) создаётся пустой список, который используется для хранения элементов стека.

Метод push() добавляет новый элемент в конец списка, тем самым помещая его на вершину стека.

Метод pop() удаляет и возвращает верхний элемент стека. Если стек пуст, то вызывается ошибка IndexError.

Метод peek() возвращает верхний элемент стека без его удаления. Если стек пуст, возвращается None.

Метод is_empty() проверяет, пуст ли стек.

Метод __len__() возвращает текущее количество элементов в стеке.

### Класс Queue

Класс Queue реализует структуру данных «очередь» с использованием collections.deque.

В конструкторе (__init__) создаётся объект deque для хранения элементов очереди.

Метод enqueue() добавляет элемент в конец очереди.

Метод dequeue() удаляет и возвращает первый элемент очереди. Если очередь пуста, то вызывается ошибка исключение IndexError.

Метод peek() возвращает первый элемент очереди без его удаления. Если очередь пуста, возвращается None.

Метод is_empty() проверяет, пуста ли очередь.

Метод __len__() возвращает количество элементов в очереди.

![Task1](images/lab10/task1.png)
![Task2](images/lab10/task2.png)

## Файл linked_list.py

В данном файле реализована структура данных односвязный список.

### Класс Node

Класс Node представляет отдельный узел односвязного списка. Он хранит значение элемента и ссылку на следующий узел списка.

### Класс SinglyLinkedList

Класс SinglyLinkedList реализует односвязный список.

В конструкторе (__init__) инициализируются ссылки на первый и последний элементы списка, а также счётчик количества элементов.

Метод append() добавляет новый элемент в конец списка.

Метод prepend() добавляет новый элемент в начало списка.

Метод insert() добавляет элемент по заданному индексу. При некорректном индексе вызывается ошибка IndexError.

Метод remove() удаляет первый найденный элемент с указанным значением.

Метод __iter__() позволяет пробегаться по элементам списка с помощью цикла for.

Метод __len__() возвращает количество элементов в списке.

Метод __repr__() возвращает строковое представление списка, удобное для вывода и отладки.

![Task3](images/lab10/task3.png)
![Task4](images/lab10/task4.png)
![Task5](images/lab10/task5.png)
![Task6](images/lab10/task6.png)

## Файл test.py

В файле test.py выполняется проверка работы всех реализованных структур данных.

![Task7](images/lab10/test.png)
![Task8](images/lab10/output.png.jpg)
